...
input_data_dir <- "data_sheets/4_802154/data/original_vectors/app5/"
file_name_prefix <- "sim802154_app5_N"
...
packet_received_app5_ts_dfs <- get_packet_received_ts_dfs(vector_data, 3, 1)

# zapisanie utworzonych szeregów do plików
packet_received_dir <- "data_sheets/4_802154/data/packet_received_vectors/app5/"
save_df_list(packet_received_app5_ts_dfs,
             packet_received_dir,
             file_name_prefix,
             file_nums,
             "packet_received.csv"
...
packet_received_plots <- create_packet_received_sizes_plots(packet_received_app5_ts_dfs,
                                                            network_sizes)

display_plots(packet_received_plots)

packet_received_comp_df <- create_comparison_df(packet_received_app5_ts_dfs,
                                                network_sizes,
                                                "total_packets_size")
...
comp_plots[[1]] <- create_multiple_ts_plot_from_df(packet_received_comp_df,
                                                   "segment",
                                                   "Zestawienie liczb otrzymywanych pakietów - aplikacja nr 5",
                                                   "czas [s]",
                                                   "liczba pakietów [B]",
